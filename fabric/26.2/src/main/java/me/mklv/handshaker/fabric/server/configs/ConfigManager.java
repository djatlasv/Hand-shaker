...
public class ConfigManager {
    ...
    public void checkPlayer(net.minecraft.server.level.ServerPlayer player, HandShakerServer.ClientInfo info) {
        checkPlayer(player, info, true); // Execute actions by default
    }
    
    public void checkPlayer(net.minecraft.server.level.ServerPlayer player, HandShakerServer.ClientInfo info, boolean executeActions) {
            ...
            if (modCfg != null) {
                ...
                if (actionDef != null && !actionDef.isEmpty()) {
                    ...
                    for (String command : actionDef.getCommands()) {
                        ...
                        if (server != null) {
                            try {
                                var parseResults = server.getCommands().getDispatcher().parse(expandedCommand, server.createCommandSourceStack());
